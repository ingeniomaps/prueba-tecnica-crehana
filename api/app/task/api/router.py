...
from app.task.domain.task_activity_log_model import TaskActivityLog
...
async def read_service(
    filters: TaskRead = Depends(),  # noqa: B008
    skip: int = 0,
    limit: int = 100,
    db: AsyncSession = Depends(get_session),  # noqa: B008
    current_user: User = Depends(get_current_user),
):
    """Obtiene una Taska de registros con filtros, paginación y límites."""
    return await read(db, filters, skip, limit)


@router.get("/{record_id}", response_model=TaskBase)
...
@router.patch("/{record_id}", response_model=TaskBase)
async def update_service(
    record_id: str,
    record_update: TaskUpdate,
    db: AsyncSession = Depends(get_session),  # noqa: B008
    current_user: User = Depends(get_current_user),
):
    """Actualiza una Taska por su ID."""
    record = await read_by_id(db, record_id)
    if not record:
        raise HTTPException(status_code=404, detail="record_not_found")
